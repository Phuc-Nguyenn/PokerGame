...
public:
  template <int N> Card &GetCard() {
    ...
    if constexpr (N == 0) return holeCards_.first;
    else if constexpr (N == 1) return holeCards_.second;
  }

  void dealCards(std::pair<Card, Card> cards) {
    holeCards_ = std::move(cards);
  }

  /**
   * Prompts the player for the next action 
   */
  std::expected<common::Action, std::error_code> PromptNextAction(common::RoundState state) {
  ...
  std::expected<common::functional::Void, std::error_code> Connect(std::unique_ptr<Connection> new_connection) {
...
private:
  ...
  /**
   * The players action this round, they might not have played yet hence optional
   */
